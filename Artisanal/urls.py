...
urlpatterns = [
    ...
    path(
        ...
        ProduitArtisanalViewSet.as_view({"get": "list"}),
        name="list-produit-artisanal-par-artisanat",
    ),
    path(
        "<int:artisanat_id>/produits/<int:pk>/",
        ProduitArtisanalViewSet.as_view({"get": "retrieve"}),
        name="detail-produit-artisanal-par-artisanat",
    ),
    # Routes pour les commandes
    path("<int:artisanat_id>/commandes/", commandeList, name="commande-list"),
    path(
        "<int:artisanat_id>/commandes/<int:produit_id>/",
        commandeListDetail,
    ...
    path(
        "panier/items/",
        ItemPanierViewSet.as_view({"get": "list"}),
        name="list-item-panier",
    ),
    path("client-panier/", PanierView.as_view(), name="panier-client"),
    # Routes pour la localisation et le responsable d'établissement
    path(
        "<int:artisanat_id>/localisations/",
        LocalisationArtisanatViewSet,
        name="list-localisation-artisanat",
    ),
    path(
        "<int:artisanat_id>/responsables/",
        ResponsableEtablissementViewSet,
        name="list-responsable-etablissement",
    ),
